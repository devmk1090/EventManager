...
public class InOutDatabase {
    ...
    public void insertRecordIn (String name, String date, String category, String relation, String money){
        try {
            db.execSQL( "insert into " + TABLE_IN_INFO + "(NAME, DATE, CATEGORY, RELATION, MONEY) values ('" + name + "','" + date + "','" + category +"','" + relation +"','" + money + "');" );
        } catch (Exception ex){
            Log.e(TAG, "Exception in executing insert SQL.", ex);
        }
    }
    public void insertRecordOut (String name, String date, String category, String relation, String money){
        try {
            db.execSQL( "insert into " + TABLE_OUT_INFO + "(NAME, DATE, CATEGORY, RELATION, MONEY) values ('" + name + "','" + date + "','" + category +"','" + relation +"','" + money + "');" );
        } catch (Exception ex){
            Log.e(TAG, "Exception in executing insert SQL.", ex);
        }
    }
    ...
    public ArrayList<InList> selectAllIn() {
        ...
        try{
            Cursor cursor = db.rawQuery("select NAME, DATE, CATEGORY, RELATION, MONEY from " + TABLE_IN_INFO, null);
            for(int i = 0; i < cursor.getCount(); i++) {
                ...
                InList info = new InList(name, date, category, relation, money);
                result.add(info);
            }
        } catch (Exception ex) {
            Log.e(TAG, "Exception in executing insert SQL.", ex);
    ...
    public ArrayList<OutList> selectAllOut() {
        ...
        try{
            Cursor cursor = db.rawQuery("select NAME, DATE, CATEGORY, RELATION, MONEY from " + TABLE_OUT_INFO, null);
            for(int i = 0; i < cursor.getCount(); i++) {
                cursor.moveToNext();
                String name = cursor.getString(0);
                String date = cursor.getString(1);
                ...
                OutList info = new OutList(name, date, category, relation, money);
                result.add(info);
            }
        } catch (Exception ex) {
            Log.e(TAG, "Exception in executing insert SQL.", ex);
    ...
    public boolean updateIn(int id, String name, String date, String category, String relation, String money) {
    ...
    public boolean updateOut (int id, String name, String date, String category, String relation, String money) {
        ContentValues cv = new ContentValues();
        cv.put("NAME", name);
        cv.put("DATE", date);
        cv.put("CATEGORY", category);
